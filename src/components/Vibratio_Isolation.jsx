...
const mlTrend = (col, data, window = 15) => {
  const vals = data.map(r => r[col]).filter(v => typeof v === 'number');
  if (vals.length < window + 3) return { direction: 'stable', changePct: 0, rate: 0 };
  const recent = vals.slice(-window);
  const prev   = vals.slice(-window * 2, -window);
  ...
  const rate = (rAvg - pAvg) / window; // units per sample
